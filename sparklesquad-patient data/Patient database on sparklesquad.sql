...
select distinct reasondescription,description from procedures
where not (Description="standard pregnancy test") and Reasondescription = "normal pregnancy";
...
where description equals to Body weigh*/
select patient,description,(value*2.2) as weight_in_Ibs from observations
...
select patient,description,(value/2.5) as "height_in_Inches" from observations
where description = "body Height";

/*6. write a query to select patient,description,reasondescription
from medications table where reasondescription is not null*/
...
Use WHERE in your join condition.Your results should contain the columns patient, marital, race, ethnicity, 
...
select p.patient,p.marital,p.race,p.ethnicity,p.gender,m.description as description,m.reasondescription as diagnosis
from patients p , medications m
where p.patient = m.patient;
...
select description,reasondescription from procedures
where not (description="standard pregnancy test") and reasondescription = "normal pregnancy";
...
select patient,description,value from observations
where value between 1 and 5 and (description = "Quality adjusted life years");
...
select patients.patient,patients.race,patients.ethnicity,patients.gender,
(procedures.description="procedure"),(procedures.reasondescription="diagonsis") from patients
left join procedures
on patients.patient=procedures.PATIENT
where ethnicity="asian_indian" AND gender= "F";
...
select patient,avg(value) as "avg_BMI",count(value) as "number_of_reading",max(value)>30 as "max_obese_BMI"
from observations_cleaned
where description= "Body mass index"
group by patient;

/*13. Write a query that returns the patient column and a count of all the allergies the patient has from allergies
 table. Group your results by patient, and order them by the number of allergies from greatest to least*/
SELECT allergies.patient,COUNT(allergies.description)
from allergies
...
select description,MAX(value),units
from observations
where description = "Body Height";

/*15. Write a query that returns the description and the number of patients 
who have an allergy to fish from the allergies table*/
select * from allergies;
select description, count(description) 
FROM allergies
WHERE description ="Allergy to fish";

/*16. Write a query against the observations_cleaned table using the general CASE expression to return the
 patient and description columns, the average of the value column, and unit column along with a new column labeled BMI category. 
...
SELECT patient,AVG(observations_cleaned.value) AS `Avg BMI`,
CASE WHEN value < 18.5 THEN "Underweight"
